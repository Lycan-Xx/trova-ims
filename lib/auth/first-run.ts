...
export async function handleFirstSignUp(
  ...
  try {
    ...
    if (inviteRes.rows.length > 0) {
      ...
      console.log(`[auth] Activated pending invite for ${normalizedEmail}`)
      return
    }
    ...
    if (userCount === 0) {
      ...
      console.log(`[auth] Created first user as owner: ${normalizedEmail} (store: ${storeId})`)
    } else {
      ...
      console.log(`[auth] Created user as cashier: ${normalizedEmail}`)
    }
  } catch (err) {
